...
function DocumentStoreContent({ params }: { params: { jobId: string } }) {
  ...
  const handleDownload = (docId: string) => {
    console.log('[DocumentStore] Download button clicked for', docId);
    handleDocumentDownload(() => {
  ...
  const renderDocumentUpload = (doc: DocumentWithStatus) => {
    if (isReportType(doc.id)) {
      const reportStatus = job
        ...
        : { isPaid: false, isCompleted: false, hasFile: false };
            ...
            <CardContent className="p-4 space-y-4">
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  Uploaded:{' '}
                  {(reportStatus as any).reportData?.completedDocument?.uploadedAt
                    ? new Date(
                        (reportStatus as any).reportData.completedDocument.uploadedAt
                      ).toLocaleDateString()
