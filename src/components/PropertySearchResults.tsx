...
export function PropertySearchResults({ planningLayers }: PropertySearchResultsProps) {
  ...
  const localEnvPlan = epiLayers.find(layer => layer.layer === "Local Environmental Plan")?.attributes || {}
  const heightOfBuilding = epiLayers.find(layer => layer.layer === "Height of Building")?.attributes || {}
