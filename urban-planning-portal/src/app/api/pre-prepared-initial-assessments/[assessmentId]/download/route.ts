  ...
  try {
    ...
    jobs.forEach(job => {
      console.log(`Job ${job.id} has assessments:`,
        Object.keys(job.purchasedPrePreparedInitialAssessments || {}))
    })
    ...
    const filePath = join(process.cwd(), 'public', 'documents', 'initial', relativePath)
    ...
    if (!existsSync(filePath)) {
      ...
      const alternativePath = join(process.cwd(), 'public', 'documents', 'initial', assessment.file.originalName)
