...
import { DesignCheckProvider, useDesignCheck } from '@shared/contexts/design-check-context';
...
import {
  DetailedDesignCheck,
  DesignCheck,
} from '@shared/components/DetailedDesignCheck';
import {
...
interface DesignCheckFormState {
...
function reportTypeToId(formType: keyof DesignCheckFormState): string {
...
function JobDesignCheck({ jobId }: { jobId: string }): JSX.Element {
  ...
  const {
    ...
    updateDesignCheck,
    saveDesignCheck,
    hasUnsavedChanges: hasUnsavedDesignCheck,
  } = useDesignCheck();
  const {
    initialAssessment,
  ...
  const [formState, setFormState] = useState<DesignCheckFormState>({
    customAssessment: {
      formData: {
        developmentType: '',
        additionalInfo: '',
  ...
  const loadFormData = (formType: keyof DesignCheckFormState): CustomAssessmentForm => {
  ...
  useEffect(() => {
    if (currentJob) {
      ...
      // Update initial assessment state from fetched job if not already modified locally
      if (!hasUnsavedInitialAssessment) {
        updateInitialAssessment(currentJob.initialAssessment || {});
      }
      ...
      updateInitialAssessment({}); // Reset initial assessment state
      setDocumentError(null);
    }
    // Dependencies: currentJob, jobId, isJobError, jobError, hasUnsavedSiteDetails, updateSiteDetails, hasUnsavedInitialAssessment, updateInitialAssessment
    else if (isJobError) {
  ...
  }, [currentJob, jobId, isJobError, jobError, hasUnsavedSiteDetails, updateSiteDetails, hasUnsavedInitialAssessment, updateInitialAssessment]);
  ...
  const handleFormChange =
    (formType: keyof DesignCheckFormState, field: keyof CustomAssessmentForm) =>
  ...
  const handleInitiatePurchase = (formType: keyof DesignCheckFormState) => {
    setFormState(prev => ({
      ...prev,
      [formType]: {
        ...prev[formType],
  ...
  const handleConfirmDetails = (formType: keyof DesignCheckFormState) => {
  ...
  const handlePayment = async (formType: keyof DesignCheckFormState) => {
    if (!jobId || !currentJob) {
      // Use jobId prop
  ...
  // Save function for Design Check
  const handleSaveDesignCheck = () => {
    if (!jobId) {
      toast({ title: 'Error', description: 'No job selected.', variant: 'destructive' });
      return;
    }
    console.log('Saving Design Check:', designCheck);
    saveDesignCheck();
  };
  ...
  const renderCustomAssessmentForm = (formType: keyof DesignCheckFormState) => {
  ...
  const handleSaveChanges = () => {
    ...
    if (hasUnsavedDesignCheck) {
      handleSaveDesignCheck(); // Triggers save for design check
      changesSaved = true;
    }
    if (hasUnsavedInitialAssessment) {
  ...
  const toggleDesignCheck = () => {
    setIsDesignCheckOpen(prev => !prev);
  };
      ...
      <div className="space-y-6">
        ...
        <div className="bg-gray-50 p-6 rounded-lg mt-8 border border-gray-200">
          <p className="text-lg mb-4">
            ...
            </br>If you are feeling brave, you can also use our chatbot to assist you in creating your Design Check. <br></br> PS, its the same as navigating to the Councils website and searching a key word in the DCP. Just all in the one spot!
          </p>
        </div>
        <div className="border rounded-lg p-4">
          <button
            onClick={toggleDesignCheck}
            className="flex items-center justify-between w-full"
          >
            <h2 className="text-xl font-semibold">Design Check</h2>
            {isDesignCheckOpen ? (
              <ChevronUp className="h-5 w-5" />
            ) : (
              <ChevronDown className="h-5 w-5" />
          ...
          {isDesignCheckOpen && (
            <div className="mt-4">
              <DetailedDesignCheck
                ...
                onDesignCheckChange={updateDesignCheck}
                readOnly={isReadOnly}
              />
            </div>
          )}
        ...
        {/* Design Check Generator Section */}
          ...
          >
            <div className="flex flex-col items-center justify-center w-full p-8">
              <p className="text-[#532200] font-semibold text-lg mb-2">Do It Yourself</p>
                <p>Use our Chatbot to assist you in creating your Design Check</p>
        ...
        <div className="bg-gray-50 p-6 rounded-lg mt-8 border border-gray-200">
          <p className="text-lg mb-4">
            We do the hard work for you. Let us prepare a Design Check for you. Purchase an assessment and we will
        ...
        {/* Save Changes Button */}
        {(Object.values(formState).some(s => s.hasUnsavedChanges) || hasUnsavedSiteDetails || hasUnsavedDesignCheck || hasUnsavedInitialAssessment) && (
...
export default function DesignCheckPage() {
  ...
  useEffect(() => {
    if (selectedJobId) {
      router.push(`/professionals/design-check?job=${selectedJobId}`, { scroll: false });
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold">Design Check</h1>
        {isLoadingJobs ? (
        ...
        <DocumentProvider jobId={selectedJobId}>
          <SiteDetailsProvider jobId={selectedJobId}>
            <InitialAssessmentProvider jobId={selectedJobId}>
              <DesignCheckProvider jobId={selectedJobId}>
                <JobDesignCheck jobId={selectedJobId} />
              </DesignCheckProvider>
          ...
          <div className="text-center text-gray-500 mt-10 border rounded-lg p-8 bg-gray-50">
            <p>Please select a job from the dropdown above to access the Design Check for your Job.</p>
          </div>

   {/* About Design Check Section */}
          <h1 className="text-3xl font-bold">About Design Check</h1>
        <div className="h-1 bg-yellow-400 w-full my-2"></div>
        <div className="bg-gray-50 p-6 rounded-lg mt-8 border border-gray-200">
          <div className="text-lg mb-4">
          <p>The Design Check platform is your comprehensive toolkit for evaluating development feasibility and compliance before you begin your project. This essential step ensures your proposed development is viable, compliant, and sustainable before significant time and resources are invested.</p>
          <br></br>
          
          <p><strong>Why Design Checks Are Essential:</strong></p>
          ...
          <p>Let our experts conduct a complete design check for you. Upload your documents, provide site details, and receive a comprehensive feasibility analysis.</p>
          <br></br>
          ...
          <p>Select a job from the dropdown above to access all features and begin your design check process.</p>
        </div>
        </div>
