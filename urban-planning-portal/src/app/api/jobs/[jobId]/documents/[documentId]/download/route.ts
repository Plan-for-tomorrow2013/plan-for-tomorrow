  ...
  try {
    ...
    if (job.purchasedPrePreparedInitialAssessments) {
      ...
      if (found && found.file) {
        ...
        const prePreparedPath = join(
          '/home/tania/urban-planning-professionals-portal/urban-planning-portal/public/documents/pre-prepared',
          originalName
        );
        try {
          ...
          console.error(
            '[DOWNLOAD API] Error reading pre-prepared initial file:',
            prePreparedPath,
            fileError
          );
    ...
    if (!document) {
      ...
      if (assessment && assessment.completedDocument) {
        ...
        // --- Check purchasedPrePreparedAssessments (regular) ---
        if (job.purchasedPrePreparedAssessments) {
          ...
          if (found && found.file) {
            ...
            try {
              ...
              console.error(
                '[DOWNLOAD API] Error reading pre-prepared file:',
                prePreparedPath,
                fileError
              );
              return NextResponse.json(
    ...
    const filePath = join(
      '/home/tania/urban-planning-portal/data/jobs',
      params.jobId,
      'documents',
      fileName
    );
