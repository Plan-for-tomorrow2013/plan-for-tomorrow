...
const prePreparedInitialAssessmentsPath = path.join(
  adminDataDir,
  'pre-prepared-initial-assessments.json'
);
...
export async function POST(request: Request) {
  try {
    ...
    let sectionIndex = prePreparedInitialAssessments.findIndex(
    ...
    await fs.writeFile(
      prePreparedInitialAssessmentsPath,
      JSON.stringify(prePreparedInitialAssessments, null, 2)
    );
