...
export async function GET(request: Request, { params }: { params: { assessmentId: string } }) {
  try {
    ...
    const jobWithAssessment = jobs.find(
      job => job.purchasedPrePreparedInitialAssessments?.[assessmentId]
    );
    ...
    const assessment = (jobWithAssessment.purchasedPrePreparedInitialAssessments || {})[
      assessmentId
    ];
