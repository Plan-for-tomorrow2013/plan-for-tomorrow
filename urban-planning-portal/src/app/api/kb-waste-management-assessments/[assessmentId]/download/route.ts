...
export async function GET(
  ...
  try {
    ...
    const jobWithAssessment = jobs.find(job =>
      job.purchasedKbWasteManagementAssessments?.[assessmentId]
    )
    ...
    const assessment = (jobWithAssessment.purchasedKbWasteManagementAssessments || {})[assessmentId]
    if (!assessment.file?.originalName) {
      return NextResponse.json({ error: 'File not found' }, { status: 404 })
    }

    // Use the same directory as kb waste management assessments
    const filePath = path.join(process.cwd(), 'public', 'documents', 'kb-waste-management-assessments', assessment.file.originalName)
