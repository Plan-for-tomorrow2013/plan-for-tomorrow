...
const kbDevelopmentApplicationAssessmentsPath = path.join(adminDataDir, 'kb-development-application-assessments.json');
// Client portal public directory (for the actual files)
const clientPublicDocumentsDir = path.join(process.cwd(), '..', 'urban-planning-portal', 'public', 'documents', 'kb-development-application-assessments');
...
export async function POST(request: Request) {
  try {
    ...
    let sectionIndex = kbDevelopmentApplicationAssessments.findIndex((section: { title: string }) => section.title === sectionTitle);
    if (sectionIndex === -1) {
    ...
    await fs.writeFile(kbDevelopmentApplicationAssessmentsPath, JSON.stringify(kbDevelopmentApplicationAssessments, null, 2));
