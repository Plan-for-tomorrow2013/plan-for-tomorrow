    ...
    let jobs = await Promise.all(
      jobFiles.map(async (file) => {
        ...
        return {
          ...
          customAssessment: job.customAssessment || null,
          statementOfEnvironmentalEffects: job.statementOfEnvironmentalEffects || null,
          complyingDevelopmentCertificate: job.complyingDevelopmentCertificate || null,
          propertyData: job.propertyData || null,
          documents: job.documents || {},
          purchasedPrePreparedAssessments: job.purchasedPrePreparedAssessments || {},
          siteDetails: job.siteDetails || null
        } as Job
      })
    )
