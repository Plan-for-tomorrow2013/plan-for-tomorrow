...
const clientPublicDocumentsDir = path.join(process.cwd(), '..', 'urban-planning-portal', 'public', 'documents', 'kb-nathers-assessments');
...
export async function GET() {
  try {
    ...
    return NextResponse.json(
      { error: 'Failed to fetch kb nathers assessments' },
      { status: 500 }
    )
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    let sectionIndex = kbNathersAssessments.findIndex((section: { title: string }) => section.title === sectionTitle);
    if (sectionIndex === -1) {
    ...
    return NextResponse.json(
      { error: 'Failed to create kb nathers assessments' },
      { status: 500 }
    );
  }
}
